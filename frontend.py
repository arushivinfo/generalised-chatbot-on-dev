...
from response_gen import compose_prompt, get_suggested_questions, get_admin_prompt_sections
from default_prompts import DEFAULT_PROMPT_SECTIONS
...
if "suggested_questions_settings" not in st.session_state:
    try:
        ...
        # Fallback to defaults if admin config fails
        st.session_state.suggested_questions_settings = {
            "enabled": True,
            "custom_prompt": DEFAULT_SUGGESTED_QUESTIONS_PROMPT,
            "max_questions": DEFAULT_SUGGESTED_QUESTIONS_COUNT
...
def load_admin_prompt_settings():
...
if "answering_templates" not in st.session_state:
    try:
        ...
        # Fallback to empty list if admin config fails
...
# store prompt blocks so edits persist (load from admin config)
...
with st.sidebar:
    ...
    # Initialize RLS values in session state
    if "rls_values" not in st.session_state:
        st.session_state.rls_values = {}
    
    # Get available user_ids from database first (needed for fallback)
    try:
    ...
    # Multi-layer RLS authentication fields
    try:
        from schema_registry import get_rls_layers, get_available_rls_fields_for_collection, get_collection_names
        rls_layers = get_rls_layers()
        
        if rls_layers:
            st.markdown("#### Authentication Values")
            st.caption("Set values for each authentication layer")
            
            # Show field availability info
            all_collections = get_collection_names()
            if all_collections:
                with st.expander("Field Availability", expanded=False):
                    for layer in rls_layers:
                        if not layer.get("enabled", True):
                            continue
                        field_name = layer.get("field_name", "")
                        display_name = layer.get("display_name", field_name)
                        
                        available_in = []
                        for collection in all_collections:
                            available_fields = get_available_rls_fields_for_collection(collection)
                            if field_name in available_fields:
                                available_in.append(collection)
                        
                        if available_in:
                            st.write(f"**{display_name}**: ✅ Available in {len(available_in)} collections")
                            st.caption(f"Collections: {', '.join(available_in)}")
                        else:
            ...
            for layer_idx, layer in enumerate(rls_layers):
                ...
                st.session_state.rls_values[field_name] = new_value
        
        else:
            # Fallback to single user_id field if no layers configured
            st.markdown("#### RLS User Selection")
            rls_user_id = st.selectbox(
                "RLS Filter User ID",
                options=[""] + available_user_ids,
                index=0,
                help="Select user_id for RLS filtering"
            )
            st.session_state.rls_values = {"user_id": rls_user_id} if rls_user_id else {}
    
    except Exception as e:
        # Fallback to original single-field approach if anything goes wrong
        st.error(f"Error loading RLS layers: {e}")
        st.markdown("#### RLS User Selection (Fallback)")
        rls_user_id = st.selectbox(
            "RLS Filter User ID",
            options=[""] + available_user_ids,
            index=0,
    ...
    active_values = {k: v for k, v in st.session_state.rls_values.items() if v and v.strip()}
    if active_values:
        if user_role in bypass_roles:
            st.success(f"🔓 Role '{user_role}' bypasses RLS - will see all data")
        else:
            st.info(f"🔒 RLS active with filters: {active_values}")
    else:
        st.caption("No RLS values set - will see all data")
...
import time
from answer_gen import process_question_complete
...
with chat_tab:
    ...
    if q:
        ...
        with st.chat_message("assistant"):
            ...
            # Process the complete question pipeline (includes suggested questions)
            answer, success = process_question_complete(
                question=q,
                resp_container=resp_container,
                step_box=step_box,
                dbg_box=dbg_box,
...
with prompt_tab:
    st.markdown("## 🔧 Admin Notice")
    st.info("Prompt settings have been moved to the admin interface for centralized management.")
    
    st.markdown("### To modify prompt settings:")
    st.markdown("1. Access the **Admin Schema UI**")
    st.markdown("2. Navigate to the **Core Rules & Prompts** tab")
    st.markdown("3. Use the **Response Generation Prompts** sub-tab")
    
    st.markdown("### Available settings in admin interface:")
    st.markdown("- **Prompt Sections**: Configure response generation templates")
    st.markdown("- **Suggested Questions**: Enable/disable and customize follow-up questions")
    st.markdown("- **Answering Templates**: Add custom response formatting templates")
    
    # Show current settings (read-only)
    with st.expander("Current Settings (Read-Only)", expanded=False):
        st.markdown("#### Current Prompt Sections")
        for section, text in st.session_state.prompt_sections.items():
            st.text_area(f"{section} (Read-Only)", value=text, disabled=True, height=100)
        
        st.markdown("#### Current Suggested Questions Settings")
        st.json(st.session_state.suggested_questions_settings)
        
        st.markdown("#### Current Answering Templates")
        if st.session_state.answering_templates:
            for i, tpl in enumerate(st.session_state.answering_templates):
                st.markdown(f"**Template {i+1}**: {tpl.get('name', 'Unnamed')} ({'Enabled' if tpl.get('enabled', False) else 'Disabled'})")
                st.text_area(f"Template {i+1} Content (Read-Only)", value=tpl.get('text', ''), disabled=True, height=80)
        else:
            st.info("No answering templates configured")
    
    # Button to refresh settings from admin
    if st.button("🔄 Refresh Settings from Admin", help="Load the latest settings from admin interface"):
        if load_admin_prompt_settings():
            st.success("✅ Successfully refreshed settings from admin interface")
            st.rerun()
        else:
            st.error("❌ Failed to refresh settings from admin interface")