            ...
            with st.expander("🤖 AI Relationship Suggestions", expanded=False):
                ...
                with col_ai1:
                    if st.button("🧠 Get AI Suggestions", type="secondary"):
                        try:
                            ...
                            for col_name in existing_colls:
                            ...
                            with st.spinner("🤖 AI analyzing collection relationships..."):
                            ...
                            if suggestions:
                                ...
                                if collection_suggestions:
                                    st.success(f"✅ Found {len(collection_suggestions)} relationship suggestions for {coll_name}")
                                else:
                                    st.info(f"💡 No relationships suggested for {coll_name}")
                            else:
                ...
                with col_ai2:
                    if hasattr(st.session_state, 'ai_suggestions') and st.session_state.ai_suggestions:
                        ...
                        if collection_suggestions and st.button("📝 Apply AI Suggestions"):
                            # Add AI suggestions to current relations
                            for suggestion in collection_suggestions:
                                # Convert suggestion to proper format based on which collection we're configuring
                                if suggestion.get('source_collection') == coll_name:
                                    # This collection is the source
                                    clean_suggestion = {
                                        'alias': suggestion.get('suggested_alias', suggestion.get('target_collection')),
                                        'ref_collection': suggestion.get('target_collection'),
                                    ...
                                    # This collection is the target, reverse the relationship
                                    clean_suggestion = {
                                        'alias': suggestion.get('suggested_alias', suggestion.get('source_collection')),
                                        'ref_collection': suggestion.get('source_collection'),
                                        'local_field': suggestion.get('target_field'),
                                        'foreign_field': suggestion.get('source_field'),
                                        'cardinality': 'many_to_one' if suggestion.get('cardinality') == 'one_to_many' else 'one_to_many' if suggestion.get('cardinality') == 'many_to_one' else 'one_to_one',
                                        'join_type': suggestion.get('join_type', 'left')
                                    }
                                
                                # Check if this relationship doesn't already exist
                                if not any(rel.get('alias') == clean_suggestion['alias'] and 
                                          rel.get('ref_collection') == clean_suggestion['ref_collection'] 
                                          for rel in st.session_state.relations_data):
                                    st.session_state.relations_data.append(clean_suggestion)
                            
                            st.success(f"✅ Applied {len(collection_suggestions)} AI suggestions")
                            st.rerun()
...
with tab_rls:
    ...
    if rls_layers:
        ...
        st.info("No authentication layers configured. Add one above to get started.")
    ...
    if rls_layers and available_collections:
        ...
        for collection in available_collections:
            collection_fields = get_available_rls_fields_for_collection(collection)
