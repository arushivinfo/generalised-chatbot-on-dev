...
def render_rls_layer_configuration(rls_config: Dict):
    """Render multi-layer RLS configuration with enforcement modes."""
    st.subheader("🔒 Multi-Layer Authentication Settings")
    
    # Initialize layers in session state if not present
    if 'rls_layers_temp' not in st.session_state:
        st.session_state.rls_layers_temp = rls_config.get("rls_layers", [])
    
    layers = st.session_state.rls_layers_temp
    
    # Track if any changes are made
    changes_made = False
    
    for i, layer in enumerate(layers):
        with st.expander(f"Layer {layer.get('order', i+1)}: {layer.get('display_name', 'Unnamed')}", expanded=True):
            col1, col2, col3 = st.columns(3)
            
            with col1:
                new_enabled = st.checkbox(
                    "Enabled", 
                    value=layer.get("enabled", False),
                    key=f"layer_enabled_{i}"
                )
                if new_enabled != layer.get("enabled", False):
                    layer["enabled"] = new_enabled
                    changes_made = True
                
                new_required = st.checkbox(
                    "Required", 
                    value=layer.get("required", False),
                    key=f"layer_required_{i}",
                    help="Must have a value to access data"
                )
                if new_required != layer.get("required", False):
                    layer["required"] = new_required
                    changes_made = True
            ...
            with col2:
                new_field_name = st.text_input(
                    ...
                    key=f"layer_field_{i}"
                )
                if new_field_name != layer.get("field_name", ""):
                    layer["field_name"] = new_field_name
                    changes_made = True
                
                new_display_name = st.text_input(
                    "Display Name", 
                    value=layer.get("display_name", ""),
                    key=f"layer_display_{i}"
                )
                if new_display_name != layer.get("display_name", ""):
                    layer["display_name"] = new_display_name
                    changes_made = True
            
            with col3:
                # ✅ Per-layer enforcement mode
                current_enforcement = layer.get("enforcement_mode", "base_only")
                new_enforcement = st.selectbox(
                    "Enforcement Mode",
                    options=["base_only", "all_involved"],
                    index=0 if current_enforcement == "base_only" else 1,
                    key=f"layer_enforcement_{i}",
                    help="""
                    **base_only**: Only applies RLS filter if the target collection directly contains this field.
                    
                    **all_involved**: Uses relationship traversal to apply filtering through related collections.
                    
                    Example: patient_id = P001 querying doctors collection:
                    • base_only: No filtering (doctors has no patient_id field)
                    • all_involved: Finds doctors via appointments relationship
                    """
                )
                if new_enforcement != current_enforcement:
                    layer["enforcement_mode"] = new_enforcement
                    changes_made = True
                ...
                new_order = st.number_input(
                    ...
                    min_value=1,
                    max_value=10,
                    value=layer.get("order", i+1),
                    key=f"layer_order_{i}"
                )
                if new_order != layer.get("order", i+1):
                    layer["order"] = new_order
                    changes_made = True
            
            new_description = st.text_area(
                "Description",
                value=layer.get("description", ""),
                key=f"layer_desc_{i}",
                height=68
            )
            if new_description != layer.get("description", ""):
                layer["description"] = new_description
                changes_made = True
            
            # Delete layer button
            if st.button(f"🗑️ Delete Layer {i+1}", key=f"delete_layer_{i}"):
                st.session_state.rls_layers_temp.pop(i)
                st.rerun()
    ...
    with col1:
        ...
        if st.button("➕ Add New Layer"):
            new_layer = {
                ...
                "enabled": False,
                "order": len(layers) + 1,
                "enforcement_mode": "base_only",
                "required": False,
                "description": ""
            }
            st.session_state.rls_layers_temp.append(new_layer)
            st.rerun()
    
    with col2:
        # Save layers button
        if st.button("💾 Save Layers", type="primary"):
            current_rls_config = get_rls_config()
            current_rls_config["rls_layers"] = st.session_state.rls_layers_temp
            set_rls_config(current_rls_config)
            st.success("✅ RLS layers saved successfully!")
            st.rerun()
    
    with col3:
        # Reset to saved state
        if st.button("🔄 Reset Changes"):
            st.session_state.rls_layers_temp = get_rls_config().get("rls_layers", [])
            st.rerun()
...
with tab_rls:
    ...
    with col1:
        ...
        enforcement_mode = st.selectbox(
            ...
            help="""
            Global enforcement strategy for RLS:
            
            **base_only**: Filters only apply to collections that directly contain the RLS field.
            Example: patient_id filter only works on collections with patient_id field.
            
            **all_involved**: Filters use relationship traversal to apply across related collections.
            Example: patient_id=P001 filters doctors by finding their appointments first.
            
            Note: Individual layers can override this global setting.
            """
        )
    ...
    render_rls_layer_configuration(rls_config)
