    ...
    try:
        ...
        if rls_user_id:
            ...
            if rls.should_bypass_rls():
                ...
                # Always add RLS filter as integer if possible and only if rls_user_id is not empty
                if rls_user_id and str(rls_user_id).isdigit():
                    rls_filter = {"user_id": int(rls_user_id)}
                elif rls_user_id:
                    # fallback for non-integer user_ids (rare)
                    rls_filter = {"user_id": rls_user_id}
                else:
                    rls_filter = None

                # Merge RLS filter into base_query
                if rls_filter:
                    if base_query:
                        base_query = {"$and": [base_query, rls_filter]}
                    else:
                        base_query = rls_filter
...
def run_search_agent(user_id: str, team_id: str,
    ...
    user_schema_section = render_schema_section_all(ALL_FIELDS, DESCRIPTIONS, OPTIONS_MAX)
    ...
    #prompt = SYSTEM_PROMPT.replace("<CONVERSATION_HISTORY>", memory_prompt)
