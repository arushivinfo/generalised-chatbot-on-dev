...
INSIGHT_MAIN_PROMPT = textwrap.dedent(
    ...
    Style:
    • Begin your response with the line `INSIGHT_CAPSULES:` followed by a JSON object (wrapped in ```json``` fences) shaped as {{{{"capsules":[{{{{"badge":"Trend","text":"≤12 word headline","detail":"longer explanation"}}}}, …]}}}}.
    • Limit to at most 5 capsules; choose badge values from {{{{"Trend","Risk","Action","Insight"}}}} (avoid "Visual") and ensure `text` is punchy and data-backed.
    • After the JSON block, present the narrative: start with a one-sentence insight that answers the request directly, followed by a bold heading and 2–3 crisp sentences.
    ...
    • Do not create a standalone section titled "Suggested Dashboard Visuals & Metrics" (or similar phrasing); weave any visual guidance directly into the narrative or bullets.
