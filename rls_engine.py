...
class RLSFieldDetector:
    ...
    OWNERSHIP_PATTERNS = {
        "exact_matches": ["user_id", "customer_id", "employee_id", "owner_id", "account_id", "member_id", "patient_id", "doctor_id"],
        "suffix_patterns": ["_user_id", "_customer_id", "_owner_id", "_account_id", "_member_id", "_patient_id", "_doctor_id"],
        "contextual_patterns": ["user.*_id", "customer.*_id", "owner.*_id", "account.*_id", "member.*_id", "patient.*_id", "doctor.*_id"]
    }
...
class RLSQueryInterceptor:
    ...
    def enhance_find_query_with_multilayer(self, collection: str, query: Dict, rls_values: Dict = None) -> Dict:
        ...
        try:
            from schema_registry import get_all_fields, load_registry, is_valid_rls_field
            reg = load_registry()
            all_fields = get_all_fields(reg)
        ...
        for layer in rls_layers:
            ...
            # Only apply filter if field exists in collection and is RLS-suitable
            if field_name not in available_fields:
                self._audit_log("RLS_SKIP", f"Field '{field_name}' not suitable for RLS in collection '{collection}' - skipping layer")
                continue
            ...
            if field_value is not None and str(field_value).strip():
                # Convert value to appropriate type based on field schema
