...
def delete_session(session_id, user_id=None, team_id=None):
    """Delete all messages for a specific session ID.
    
    Args:
        session_id: The ID of the session to delete
        user_id: Optional user ID to ensure ownership (security)
        team_id: Optional team ID to ensure ownership (security)
    ...
    # Add optional filters for security
    if user_id is not None:
        query["user_id"] = user_id
    if team_id is not None:
        query["team_id"] = team_id
