...
if isinstance(view_param, list):
    active_view = view_param[0] if view_param else "studio"
elif isinstance(view_param, str):
    active_view = view_param
else:
    active_view = "studio"

if active_view == "dashboard":
    insight_runs = st.session_state.setdefault("insight_runs", [])
    st.session_state["dashboard_runs"] = insight_runs

    if "dashboard_session_id" not in st.session_state:
        st.session_state["dashboard_session_id"] = st.session_state.get("sid", "")

    def _back_to_studio() -> None:
    ...
    render_dashboard_view(
        ...
        session_id=st.session_state.get("dashboard_session_id", ""),
        allow_generate=True,
        show_back_button=True,
        back_callback=_back_to_studio,
    )
    st.stop()
...
st.session_state["dashboard_runs"] = st.session_state.insight_runs
insight_runs = st.session_state.insight_runs
...
if submitted:
    ...
    if not cleaned_request:
        ...
        insight_runs[idx]["status"] = "ready"
        insight_runs[idx]["result"] = result
...
for run_idx, run in enumerate(insight_runs):
    display_idx = len(insight_runs) - run_idx
...
if insight_runs:
    ...
    if st.button("Open Dashboard Studio", use_container_width=True):
        st.query_params["view"] = "dashboard"
        st.rerun()
