...
def get_rls_config(reg: Dict | None = None) -> Dict:
    """Get the Row-Level Security configuration with enhanced multi-layer support."""
    reg = reg or load_registry()
    if "rls_config" not in reg:
        reg["rls_config"] = {
            "enabled": False,
            "global_enforcement_mode": "base_only",  # Global default
            ...
            "rls_layers": [
                {
                    ...
                    "enabled": True,
                    "order": 1,
                    "enforcement_mode": "base_only",  # ✅ Layer-specific enforcement
                    "required": True,
                    "description": "Primary user isolation layer"
                },
                {
                    "field_name": "team_id",
                    ...
                    "enabled": True, 
                    "order": 2,
                    "enforcement_mode": "all_involved",  # ✅ Different enforcement per layer
                    "required": False,
                ...
                },
                {
                    ...
                    "enabled": False,
                    "order": 3,
                    "enforcement_mode": "all_involved",  # ✅ Session isolation
                    "required": False,
                    "description": "Session-specific data filtering"
                }
            ],
            "collections": {}
        }
    return reg["rls_config"]
...
def is_valid_rls_field(field_info: Dict) -> bool:
    """Check if a field is suitable for RLS filtering."""
    field_name = field_info.get("name", "").lower()
    field_type = field_info.get("type", "")
    
    # Include all string fields that might be used for RLS
    return field_type == "string" or "_id" in field_name
...
def get_response_prompt_config(reg: Dict | None = None) -> Dict:
    """Get the response prompt configuration from admin settings."""
    if reg is None:
        reg = load_registry()
    
    # Import centralized defaults
    from default_prompts import get_default_response_prompt_config
    
    # Return existing config or fall back to centralized defaults
    config = reg.get("response_prompt_config")
    if not config:
        return get_default_response_prompt_config()
    
    return config

def set_response_prompt_config(prompt_sections: Dict, suggested_questions_settings: Dict, answering_templates: list) -> Dict:
    """Set the response prompt configuration in admin settings."""
    reg = load_registry()
    reg["response_prompt_config"] = {
        "prompt_sections": prompt_sections,
        "suggested_questions_settings": suggested_questions_settings,
        "answering_templates": answering_templates
    }
    save_registry(reg)
    return reg
